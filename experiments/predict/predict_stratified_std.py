...
#### A1
treatment = 1
df_A1 = df[df['t'] == treatment].copy()
# describe the distritbution of 't' in df_A1
#print(df_A1['t'].describe())
# print column names of df_A1
num_bins = NUM_BINS
processor = DataProcessor(df_A1)
processor.detect_variable_type('t')
processor.sample_variables()
processor.bin_continuous_variables(num_bins)
tensor, feature_names = processor.create_tensor()
binary_dims, continuous_dims = processor.generate_dimensions()
...
scenarios = {
    "train": (val_loader, val_data),  # Model trained on training data predicting on validation data
    "val": (train_loader, train_data)  # Model trained on validation data predicting on training data
}

# Combine predictions from both models
df_stratified_std_A1 = pd.DataFrame()

for model_type, (loader, data) in scenarios.items():
    #model.load_state_dict(torch.load(model_path))
    model_path = (f"experiments/model/{dataset_type}/model_treatment{treatment}_{dataset_type}_{model_type}_sample{N_SAMPLE}_"
                  f"epoch{N_EPOCHS}_lr{LEARNING_RATE:.4f}_wd{WEIGHT_DECAY:.4f}_dr{DROPOUT_RATE:.4f}_bs{BATCH_SIZE}_ed{EMBEDDING_DIM}_nh{N_HEAD}.pth")

    df = predict_and_create_df(model, model_path, device, trainer, loader, data, processor, feature_names, dataset_type)
    #df['y'] = dataset['y']
    df['data_source'] = model_type
    df_stratified_std_A1 = pd.concat([df_stratified_std_A1, df])
...
df = dataframe.iloc[:, :-3].copy()
treatment = 0
df_A0 = df[df['t'] == treatment].copy()
# describe the distritbution of 't' in df_A1
#print(df_A1['t'].describe())
# print column names of df_A1
num_bins = NUM_BINS
processor = DataProcessor(df_A0)
processor.detect_variable_type('t')
processor.sample_variables()
processor.bin_continuous_variables(num_bins)
tensor, feature_names = processor.create_tensor()
...
# Combine predictions from both models
df_stratified_std_A0 = pd.DataFrame()
...
for model_type, (loader, data) in scenarios.items():
    ...
    #df['y'] = dataset['y']
    df['data_source'] = model_type
    df_stratified_std_A0 = pd.concat([df_stratified_std_A0, df])

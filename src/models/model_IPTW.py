...
wandb.init(
    ...
    project="dag-aware-transformer-IPTW",
    name = f"4 confounders (sex, age, smoke, asthma), 1 treatment (qsmk), 1 outcome (death), with interaction",
...
nconfounder = 4
df.to_excel(f"data/dataL{nconfounder}+{n_samples}+epochs{n_epochs}+lr{learning_rate}+IPTW-with-interaction.xlsx", index=False)

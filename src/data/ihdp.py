...
import numpy as np
import pandas as pd
from src.utils import download_dataset, unzip
...
def data_split_ihdp(data, dag):
    ...
    df_w = pd.DataFrame(w, columns=[f'x{i + 1}' for i in range(25)])
    df_t = pd.DataFrame(t, columns=['t'])
    df_y = pd.DataFrame(y, columns=['y'])
    df_ites = pd.DataFrame(ites, columns=['ite'])

    # Concatenate along the columns
    data = pd.concat([df_w, df_t, df_y, df_ites], axis=1)
...
if __name__ == "__main__":
    ...
    # Load the first instance of the IHDP dataset (all returns the total sample size)
    data = load_ihdp(split='all', i=0, return_ites=True)

    train_data, val_data, test_data = data_split_ihdp(data, dag)
